...
class LoginController: UIViewController {
    ...
    private let dontHaveAccountButton: UIButton =  {
        let button = Utilites().attributedButton("Dont't have an account?", " Sign Up")
    ...
    @objc func handleLogin() {
        ...
        AuthServies.shared.logUserIn(withEmail: email, password: password) { result, error in
            if let error = error {
                debugPrint(error.localizedDescription)
                return
            }
            debugPrint("DEBUG: Successfully log in..")
